...
└── dist/
    ...
    └── test.exe            # Windows 単一実行ファイル（ビルド生成物）
```
...
**macOS / Linux（開発マシン）:**
```bash
cat micro.sfx dist/app.phar > dist/test.exe
```

**Windows:**
```cmd
copy /b micro.sfx + dist\app.phar dist\test.exe
```
...
`dist\test.exe` をダブルクリックすると、ソケットベースの HTTP サーバーが起動しブラウザが自動で開きます。

> **仕組み**: `test.exe` = `micro.sfx`（PHP ランタイム）+ `app.phar`（アプリコード）。  
...
./build.sh
# または手動で:
cat micro.sfx dist/app.phar > dist/test && chmod +x dist/test
```
...
| micro.sfx モード | `test.exe` / `test` バイナリ | ソケットサーバー（`stream_socket_server`） |
